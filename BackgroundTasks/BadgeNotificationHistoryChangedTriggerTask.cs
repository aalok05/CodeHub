...
namespace BackgroundTasks
{
	public sealed class BadgeNotificationHistoryChangedTriggerTask : IBackgroundTask
	{
		public void Run(IBackgroundTaskInstance taskInstance)
		{
			...
			var TNList = ToastNotificationManager.History.GetHistory();

			var badgeContent = new BadgeNumericNotificationContent((uint)TNList.Count);
	...
	public sealed class BadgeNumericNotificationContent
	{
		...
		/// Numbers greater than 99 are allowed, but will be displayed as "99+".
		/// </summary>
		private uint m_umber { get; set; } = 0;
		...
		{
			return string.Format("<badge version='{0}' value='{1}'/>", Util.NOTIFICATION_CONTENT_VERSION, m_umber);
		}
