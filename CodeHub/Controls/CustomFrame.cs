...
namespace CodeHub.Controls
{
	...
	public class CustomFrame : Frame
	{
		...
		private Task GetContentForwardOutStoryboard() 
			=> FrameworkContent
				.StartCompositionFadeScaleAnimationAsync(1, 0, 1, 1.02f, ContentAnimationDuration, null, 0, EasingFunctionNames.CircleEaseIn);
		...
		private Task GetContentForwardInStoryboard() 
			=> FrameworkContent
				.StartCompositionFadeScaleAnimationAsync(0, 1, 0.98f, 1, ContentAnimationDuration, null, 0, EasingFunctionNames.CircleEaseOut);
		...
		private Task GetContentBackOutStoryboard() 
			=> FrameworkContent
				.StartCompositionFadeScaleAnimationAsync(1, 0, 1, 0.98f, ContentAnimationDuration, null, 0, EasingFunctionNames.CircleEaseIn);
		...
		private Task GetContentBackInStoryboard() 
			=> FrameworkContent
				.StartCompositionFadeScaleAnimationAsync(0, 1, 1.02f, 1, ContentAnimationDuration, null, 0, EasingFunctionNames.CircleEaseOut);
		...
		{
			var eventPending = IsEmpty;
			var result = base.Navigate(pageType, parameter);
			if (eventPending)
			{
		...
		public new async Task<bool> Navigate(Type pageType, [Optional] object parameter)
		{
			...
			async void finishNavigation(bool empty)
			{
