...
namespace CodeHub.Models
{
    ...
    public struct WrappedWebResult<T> : IEquatable<WrappedWebResult<T>>, IEquatable<T> where T : class
    {
        ...
        public bool Equals(WrappedWebResult<T> other) => Equals(other.Result);
        ...
        public static implicit operator T(WrappedWebResult<T> wrappedResult) => wrappedResult.Result;

        // Implicit converter for successful results
        public static implicit operator WrappedWebResult<T>([NotNull] T result) => new WrappedWebResult<T>(result, WebRequestStatus.RequestCompleted);

        // Implicit converters for faulted results
        public static implicit operator WrappedWebResult<T>(WebRequestStatus status) => new WrappedWebResult<T>(null, status);
    }
    ...
    public struct WrappedHTTPWebResult<T> : IEquatable<WrappedHTTPWebResult<T>>, IEquatable<T> where T : class
    {
        ...
        // Implicit cast for the inner result
        public static implicit operator T(WrappedHTTPWebResult<T> wrappedResult) => wrappedResult.Result;

        // Implicit converter for successful results
        public static implicit operator WrappedHTTPWebResult<T>([CanBeNull] T result) => new WrappedHTTPWebResult<T>(result, HttpStatusCode.Ok, null);

        // Implicit converters for faulted results
        public static implicit operator WrappedHTTPWebResult<T>(HttpStatusCode status) => new WrappedHTTPWebResult<T>(null, status, null);

        // Failed web call
        public static implicit operator WrappedHTTPWebResult<T>([NotNull] Exception e) => new WrappedHTTPWebResult<T>(null, null, e);
    }
}